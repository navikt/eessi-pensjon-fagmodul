...
class VedleggController(private val vedleggService: VedleggService,
    ...
    fun putVedleggTilDokument(@PathVariable("aktoerId", required = true) aktoerId: String,
                              ...
                              @PathVariable("variantFormat", required = true) variantFormat : String) : ResponseEntity<FrontEndResponse<Unit>> {
        ...
        return try {
            ...
            return ResponseEntity.ok(FrontEndResponse(result = null, status = HttpStatus.OK.name))
        } catch (ex: Exception) {
