...
internal class PrefillControllerTest {
    ...
    fun `createBuc run ok and return id`() {
        ...
        every { mockEuxKlient.getBucJsonAsNavIdent(any()) } returns gyldigBuc
...
//        val buc : Buc =  mapJsonToAny(gyldigBuc)
        ...
        every { mockEuxKlient.createBuc(P_BUC_03.name) } returns "1231231"
        every { mockEuxKlient.getBucJsonAsNavIdent(any()) } returns gyldigBuc
    ...
    fun `call addInstutionAndDocument mock adding two institusjon when X005 exists already as new will throw bad request`() {
        ...
        every {
            personService.hentIdent(
                eq(IdentGruppe.FOLKEREGISTERIDENT),
                any<AktoerId>()
            )
        } returns NorskIdent("12345")

        val mockParticipants = listOf(
            Participant(
                role = "CaseOwner",
                organisation = Organisation(countryCode = "NO", name = "NAV", id = "NAV")
            )
        )
        val mockBuc = Buc(id = "23123", processDefinitionName = "P_BUC_01", participants = mockParticipants)
        mockBuc.documents =
            listOf(createDummyBucDocumentItem(), DocumentsItem(type = SedType.X005, status = "new", direction = "OUT"))
        ...
        verify(exactly = 1) { mockEuxKlient.getBucJsonAsNavIdent(any()) }
    ...
    fun `Ved kall til addInstutionAndDocument med en institusjon så skal vi få en preutfylt P8000 med en deltaker`() {
        ...
        every { prefillKlient.hentPreutfyltSed(any()) } returns javaClass.getResource("/json/nav/P8000_NO-NAV.json")!!
            .readText()
        ...
        val apiRequest = ApiRequest(
            ...
            institutions = listOf(
                InstitusjonItem(
                    country = "SE",
                    institution = "SE:ACC2001",
                    name = "The Swedish Pensions Agency",
                    acronym = "PM Sverige"
                ),
                InstitusjonItem(
                    country = "FI",
                    institution = "FI:0100222222",
                    name = "ACC_The Social Insurance Institution of Finland",
                    acronym = "KELA, FPA"
                )
            )
        )
        ...
        every { mockEuxKlient.opprettSed(any(), any()) } returns BucSedResponse(
            euxCaseId,
            "0aa6a0d67e4046e38aa126987ab2763f"
        )
    ...
    fun `call addInstutionAndDocument mock adding two institusjon when we are not CaseOwner badrequest execption is thrown`() {
        ...
        val mockParticipants = listOf(
            Participant(
                role = "CaseOwner",
                organisation = Organisation(countryCode = "SE", name = "SE", id = "SE")
            )
        )
        val mockBuc = Buc(id = "23123", processDefinitionName = "P_BUC_01", participants = mockParticipants)
        mockBuc.documents = listOf(
            createDummyBucDocumentItem(),
            DocumentsItem(type = SedType.X005, status = "empty", direction = "OUT")
        )
        mockBuc.actions = listOf(ActionsItem(operation = ActionOperation.Send))
        ...
        verify(exactly = 1) { mockEuxKlient.getBucJsonAsNavIdent(euxCaseId) }
                ...
                sak = Navsak(
                    leggtilinstitusjon = Leggtilinstitusjon(
                        institusjon = InstitusjonX005(
                            id = newParticipants.institution,
                            navn = newParticipants.name ?: ""
                        )
                    )
                )
            )
    ...
    fun `call addInstutionAndDocument add newInstitusjonItem on empty buc NAV is caseOwner`() {
        ...
        every { mockEuxKlient.opprettSed(any(), eq(euxCaseId)) } returns BucSedResponse(euxCaseId, "1")

        prefillController.addInstutionAndDocument(apirequest)

        verify(exactly = 1) { mockEuxKlient.getBucJsonAsNavIdent(any()) }
        verify(exactly = 1) { mockEuxKlient.opprettSed(any(), euxCaseId) }
    ...
    fun `call addInstutionAndDocument add newInstitusjonItem on empty buc rina2020 NAV is caseOwner`() {
        ...
        every { mockEuxKlient.getBucJsonAsNavIdent(any()) } returns mockBuc.toJson() andThen mockDocBuc.toJson()
        every { prefillKlient.hentPreutfyltSed(any()) } returns SED(type = dummyPrefillData.sedType).toJson()
        every { mockEuxKlient.putBucMottakere(any(), any()) } returns true
        every { mockEuxKlient.opprettSed(any(), eq(euxCaseId)) } returns BucSedResponse(
            euxCaseId,
    ...
    fun `call addInstutionAndDocument ingen ny Deltaker kun hovedsed`() {
        ...
        val dummyPrefillData =
            ApiRequest.buildPrefillDataModelOnExisting(apiRequest, PersonInfo(NorskIdent("12345").id), null)
...
//        every { mockEuxInnhentingService.getBuc(euxCaseId) } returns mockBuc
//        every { mockEuxKlient.createBuc(P_BUC_03.name) } returns "1231231"
        every { mockEuxKlient.getBucJsonAsNavIdent(any()) } returns mockBuc.toJson()
        every { prefillKlient.hentPreutfyltSed(any()) } returns SED(type = dummyPrefillData.sedType).toJson()
        every { mockEuxKlient.opprettSed(any(), eq(euxCaseId)) } returns BucSedResponse(euxCaseId, "3123123")

        prefillController.addInstutionAndDocument(apiRequest)

        verify(exactly = noNewParticipants.size + 1) { mockEuxKlient.opprettSed(any(), euxCaseId) }
        verify(exactly = 1) { mockEuxKlient.getBucJsonAsNavIdent(any()) }
        verify(exactly = 1) { prefillKlient.hentPreutfyltSed(any()) }
        verify(exactly = 1) { personService.hentIdent(any(), any<AktoerId>()) }
    ...
    fun `call addDocumentToParent ingen ny Deltaker kun hovedsed`() {
        ...
        val mockBuc = Buc(
            id = "23123", processDefinitionName = "P_BUC_01",
            ...
            actions = listOf(
                ActionsItem(
                    documentType = SedType.P8000,
                    documentId = parentDocumentId,
                    operation = ActionOperation.Read
                ), ActionsItem(documentType = SedType.P9000, documentId = "3123123", operation = ActionOperation.Create)
            )
        )
...
//        every { mockEuxKlient.createBuc(P_BUC_03.name) } returns "1231231"
        ...
        every { mockEuxKlient.opprettSvarSed(any(), euxCaseId, parentDocumentId) } returns BucSedResponse(
            euxCaseId,
            "3123123"
        )
        ...
        verify(exactly = 2) { mockEuxKlient.getBucJsonAsNavIdent(any()) }
        verify(exactly = 1) { mockEuxKlient.opprettSvarSed(any(), any(), any()) }
        verify(exactly = 1) { personService.hentIdent(any(), any<AktoerId>()) }
        verify(exactly = 1) { prefillKlient.hentPreutfyltSed(any()) }
    ...
    fun `Gitt det opprettes en SED P10000 på tom P_BUC_06 Så skal bucmodel hents på nyt og shortDocument returneres som response`() {
        ...
        every { mockEuxKlient.getBucJsonAsNavIdent(euxCaseId) } returns mockBucJson andThen mockBucJson2
        val newParticipants = listOf(
        ...
        every { mockEuxKlient.opprettSed(any(), euxCaseId) } returns BucSedResponse(
            euxCaseId,
            "58c26271b21f4feebcc36b949b4865fe"
        )
        every { mockEuxKlient.putBucMottakere(any(), any()) } returns true

        val result = prefillController.addInstutionAndDocument(apiRequest)

        verify(exactly = 1) { mockEuxKlient.opprettSed(any(), eq(euxCaseId)) }
        verify(exactly = 2) { mockEuxKlient.getBucJsonAsNavIdent(eq(euxCaseId)) }
...
//        every{mockEuxInnhentingService.getBuc(euxCaseId)} returns mockBuc
        ...
        val apiRequest = apiRequestWith(euxCaseId, newParticipants, buc = P_BUC_01)
        val dummyPrefillData = ApiRequest.buildPrefillDataModelOnExisting(
            apiRequestWith(euxCaseId),
            PersonInfo(NorskIdent("12345").id),
            null
        )
...
//        every{mockEuxInnhentingService.getBuc(euxCaseId)} returns mockBuc
        ...
        every { prefillKlient.hentPreutfyltSed(any()) } returns SED(type = dummyPrefillData.sedType).toJson()
        every { mockEuxKlient.opprettSed(any(), eq(euxCaseId)) } throws SedDokumentIkkeOpprettetException("Expected!")
